...
def __validate_inputs(src1, src2):
    ...
    if os.path.isdir(src1) is True and os.path.isdir(src2) is False:
        raise ValueError(
            "src1 is a directory but src2 is not. Check to see if src2 exists and is a dir" " and not a file"
        )

    if os.path.isdir(src1) is False and os.path.isdir(src2) is True:
        raise ValueError(
            "src2 is a directory but src1 is not. Check to see if src1 exists and is a dir" " and not a file"
        )
    ...
    if os.path.isfile(src1) is True and os.path.isfile(src2) is True:
        ...
        if src1_ext.lower() != src2_ext.lower():
            raise ValueError(
                "The two files submitted need the extensions to match or they will obviously fail"
            )
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="This tool can be used to compare exact contents of all"
