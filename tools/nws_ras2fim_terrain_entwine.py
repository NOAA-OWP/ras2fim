...
gdf_entwine_footprints = gpd.overlay(gdf_entwine_footprints, gdf_boundary_lambert, how="intersection")
...
pipeline_dem_ground = {
    "pipeline": [
        ...
        {"type": "filters.range", "limits": "Classification[2:17]"},
        {"type": "filters.range", "limits": "Classification![3:8]"},
        {"type": "filters.range", "limits": "Classification![17:17]", "tag": "GroundOnly"},
        {
...
with rasterio.open(str_dem_ground) as src:
    ...
    arr_filled = fillnodata(
        arr, mask=src.read_masks(1), max_search_distance=FLT_SEARCH_DIST, smoothing_iterations=0
    )
...
lidar_dem_local_proj.rio.to_raster(str_dem_out, compress="LZW", dtype="float32")
