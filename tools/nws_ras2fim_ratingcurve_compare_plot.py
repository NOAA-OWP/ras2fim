...
list_csv_files = [os.path.join(d, x) for d, dirs, files in os.walk(path) for x in files if x.endswith(".csv")]
...
def fn_create_rating_curve(
    ...
    fig.suptitle(
        "HYDRO ID: " + str_hydro_id_fn + "\nFEATURE ID: " + str_feature_id_fn, fontsize=10, fontweight="bold"
    )
    ...
    if len(df_single_hydro_id) > 0:
        ...
        ax.text(
            ...
            0.27,
            "HydroID: slope="
            + str("{:.2f}".format(hydro_id_slope))
            + "%"
            + " length="
            + str("{:.2f}".format(hydro_id_length))
            + " mi",
    ...
    ax.text(
        0.98,
        0.16,
        "Mean absolute depth difference: " + str("{:.2f}".format(flt_madd)) + "ft",
        verticalalignment="bottom",
        horizontalalignment="right",
        backgroundcolor="w",
        transform=ax.transAxes,
    ...
    ax.text(
        0.98,
        0.19,
        "Mean normalized RMSE: " + str("{:.1f}".format(flt_norm_rmse)) + "%",
        verticalalignment="bottom",
        horizontalalignment="right",
        backgroundcolor="w",
        transform=ax.transAxes,
    ...
    ax.text(
        0.98,
        0.22,
        "Percent bias: " + str("{:.1f}".format(flt_percent_bias)) + "%",
        verticalalignment="bottom",
        horizontalalignment="right",
        backgroundcolor="w",
        transform=ax.transAxes,
    ...
    plt.plot(
        list_int_step_flows_fn, list_step_profiles_fn, label="HAND FIM 3_0_15_2 (per Hydro_ID)"
    )  # creates the line (HAND)
    plt.plot(
        list_ble_flows_fn, list_ble_profiles_fn, label="BLE2FIM (per Feature_ID)"
    )  # creates the line (ble2fim)
    ...
    plt.plot(
        list_int_step_flows_fn, list_step_profiles_fn, "bd", markersize=1
