...
S3, models_catalog.csv and models folders, you are welcome to use it. We can not grant access to
...
Features for this tool include:
    ...
    - A "verbose" flag can be optionally added as an argument for additional processing details
         (note: don't over use this as it can make errors harder to find). To find an error, simply search
    ...
    - This script can find huc numbers in the models catalog "hucs" field regardless of string format in
          that column. It does assume that models catalog has ensure that leading zero's exist.

    - This script further filters model records to the matching crs value. Why? ras2fim.py has an input
         param for its crs (-p) and only models matching that crs can be processed at one time. If we
         have one HUC that has some models in one projection and some models in another projection, they
         must be processed via ras2fim.py separately with separate output folders.

    - As the models folders are about to be downloaded, all subfolders in the local "models" folder will
         first be removed, as ras2fim.py processes all folders in that directory.

    - It creates a filtered copy of the S3 OWP_ras_models_catalog.csv showing all folders downloaded,
         if they were successful or not and if not, why. Local models catalog file name
         will be "OWP_ras_models_catalog_{HUC}.csv"
...
class Get_Ras_Models_By_Catalog:
    ...
    # Default values listed in "__main__"  but also here in case code calls
    #    direct.. aka. not through "__main__"
    def get_models(
        self,
        ...
        - Current filters when used against the OWP_ras_models_catalog.csv:
              ...
              - 'hucs' column includes the provided huc_number. Note.. more than one huc can be in
                 that column.
        ...
        - projection (string): To calculate which model folders are to be downloaded, you need
            the HUC number and the case-senstive CRS number.
            The projection value must match the 'crs' column in the master copy in
            S3  (OWP_ras_models_catalog.csv)
        ...
        - All of the filtered actual folders found in the s3 OWP_ras_models based on the models catalog are
            placed in the local emptied (folders only) target_owp_ras_models_path
            (assuming not overwritten to list only).

        - A filtered copy of the "OWP_ras_models_catalog.csv" from S3 will be placed in as set by
            target_owp_ras_models_csv_file and includes list of all downloaded files and their attributes.
            If not overridden, the name will become OWP_ras_models_catalog_{HUC}.csv. This file will
            overwrite a pre-existing file of the same name. The new OWP_ras_models_catalog_{HUC}.csv is
            required for ras2fim.py processing.
        ...
        self.log_append_and_print("****************************************")
        self.log_append_and_print(
            f"Get ras models folders from s3 started: {dt_string}"
        )
        ...
        try:
            ...
            df_all = pd.read_csv(
                self.s3_path_to_catalog_file, header=0, encoding="unicode_escape"
            )

            if df_all.empty:
                self.log_append_and_print(
                    "The model catalog appears to be empty or did not load correctly"
                )
                return
            ...
            if self.is_verbose is True:
                self.log_append_and_print(
                    f"models catalog raw record count = {len(df_all)}"
                )
            ...
            if df_huc.empty is True:
                self.log_append_and_print(
                    f"No valid records return for {self.huc_number}. {filter_msg}"
                )
                return
            ...
            if self.df_filtered.empty is True:
                self.log_append_and_print(
            ...
            self.log_append_and_print(
                f"Filtered model catalog saved to : {self.target_filtered_csv_path}"
            )
            if self.list_only is False:
                self.log_append_and_print(
                    "Note: This csv represents all filtered models folders that are pending to \n"
                    "be downloaded. The csv will be updated with statuses after downloads are complete."
                )
            ...
            # ----------
            # If inc_download_folders, otherwise we just stop.  Sometimes a list is wanted 
            # but not the downloads
            if self.list_only is False:
                # loop through df_huc records and using name, pull down
                # TODO: Add a progress bar and multi proc, BUT....
                # S3 does not like two many simulations downloads at a time. It has to do with the
                # network speed of your machine.
            ...
            if self.list_only is True:
                self.log_append_and_print(
                    "List only as per (-f) flag - no downloads attempted"
                )
                self.log_append_and_print(
                    f"Number of model folders which would be attempted to downloaded is {len(self.df_filtered)}"
                )
            else:
                self.log_append_and_print(
        ...
        if (not self.df_filtered.empty is False) and (self.list_only is False):
            self.df_filtered.to_csv(self.target_filtered_csv_path, index=False)
    ...
    def __validate_inputs(
        ...
        # ---------------
        crs_number, is_valid, err_msg = val.is_valid_crs(
            projection
        ...
        if list_only is False:
            if os.path.exists(self.target_owp_ras_models_path):
                ...
                # quick time out here so sometimes mkdir can fail if rmtree isn't done.
                time.sleep(2)  # 2 seconds

            os.mkdir(self.target_owp_ras_models_path)

        # the string may or may not have the [] and that is ok.
        self.target_filtered_csv_path = target_owp_ras_models_csv_file.replace(
            "[]", huc_number
        )
        ...
        self.src_owp_model_folder_path = (
            self.bucket_name + "/" + self.src_owp_model_folder_path
        )
        ...
        self.log_append_and_print(
            f"Source download path for models is {self.src_owp_model_folder_path}"
        )
        self.log_append_and_print(
            f"Target file name and path for the filtered csv is {self.target_filtered_csv_path}"
        )
        self.log_append_and_print(
            f"Target path for models is {self.target_owp_ras_models_path}"
        )
        self.log_append_and_print("")
    ...
    def download_files(self, folder_list):
        ...
        for folder_name in folder_list:
            ...
            rowIndexes = self.df_filtered.index[
                self.df_filtered["final_name_key"] == folder_name
            ].tolist()
            if len(rowIndexes) != 1:
                msg =  "Sorry, something went wrong looking the specific record" \
                      f"with a final_name_key of {folder_name}"
            ...
            if s3.exists(src_path) is False:
                ...
                self.log_append_and_print(f".... {msg}")
                progress_msg = (
                    f">>> {num_processed} of {self.num_pending_downloads} processed"
                )
                self.log_append_and_print(progress_msg)

                # update the df (csv) to show it failed and why
                self.df_filtered.loc[
                    rowIndex, MODELS_CATALOG_COLUMN_DOWNLOAD_SUCCESS
                ] = "False"
                self.df_filtered.loc[
                    rowIndex, MODELS_CATALOG_COLUMN_DOWNLOAD_FAIL_REASON
                ] = msg
                continue

            target_path = os.path.join(self.target_owp_ras_models_path, folder_name)
            self.log_append_and_print(
                f"== {folder_name} - Downloading to path = {target_path}"
            )
    ...
    def log_append_and_print(self, msg):
        ...
        if hasattr(self, "log_file_msg") is False:
            self.log_file_msg = ""
    ...
    def save_logs(self):
        ...
        if hasattr(self, "log_folder_path") is False:
            return
        ...
        log_file_path = os.path.join(
            ...
            f"get_ras_models_{self.huc_number}-{file_dt_string}.log",
        )
...
if __name__ == "__main__":
    ...
    # ***********************
    # This tool is intended for NOAA/OWP staff only as it requires access to an AWS S3 bucket with
    ...
    # python3 ./tools/get_models_by_catalog.py
    #         -s s3://xyz/OWP_ras_models/OWP_ras_models_catalog.csv
    #         -u 12090301 -p ESRI:102739

    # Sample Usage with most params
    # python3 ./tools/get_models_by_catalog.py -u 12090301
    #          -s s3://xyz/OWP_ras_models/OWP_ras_models_catalog.csv
    #          -tm c:\\ras2fim\\ras_models
    #          -tcsv c:\\ras2fim\ras_models\12090301_models_catalog.csv
    #          -v -p ESRI:102739

    # Sample Usage to get just the list and not the model folders downloaded (and most defaults)
    # python3 ./tools/get_models_by_catalog.py
    #          -s s3://xyz/OWP_ras_models/OWP_ras_models_catalog.csv
    #          -u 12090301 -p EPSG:2277 -f
    ...
    # e.g.  s3://(some bucket)/OWP_ras_models/OWP_ras_models_catalog.csv with a "models" folder
    #  beside it with all of the "model" folders under that.
    ...
    #                                                   /etc
    # And of course, your path doesn't have to match, but the pattern of the csv, "models" folder,
    # and subfolders is critical
