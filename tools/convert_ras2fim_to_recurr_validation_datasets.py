    ...
    for flow_file in listdir(recurrence_dir):
        ...
        for rc_file in feature_ids_rc:
            ...
            interp_depth = np.interp(
                val_discharge["discharge"],
                ras_rc["discharge_cms"],
                ras_rc["stage_m"],
                left=np.nan,
                right=np.nan,
            )
            ...
            if not pd.isna(interp_depth):
                ...
                if (benchmark is not None) and (
                    benchmark - interp_depth <= 0.3048
                ):  # see https://github.com/NOAA-OWP/cahaba/wiki/Evaluating-HAND-Performance, Fig A.5
