...
def fn_calculate_terrain_stats(str_geom_hdf_path, str_projection_path, str_terrain_path):
    ...
    for i in range(len(arr_pnts_per_xs)):
        str(arr_xs_attrib[i][2].decode("UTF-8"))
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        help="REQUIRED: shapefile on HEC-RAS projection:"
        r"Example: C:\shapes_from_conflation\10170204_huc_12_ar.shp",
        required=True,
        metavar="FILE",
        type=lambda x: fn_is_valid_file(parser, x),
    )
    ...
    parser.add_argument(
        ...
        help="REQUIRED: path to the terrain (geotiff):"
        r"Example: C:\output_folder\terrain_from_usgs\101702040205.tif",
        required=True,
        metavar="FILE",
        type=lambda x: fn_is_valid_file(parser, x),
    )
