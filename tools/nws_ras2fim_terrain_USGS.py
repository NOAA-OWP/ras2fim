...
# USGS WCS endpoint for each polygon.  This samples in tiles to the requested
...
# -------------------------------------------------
# TODO: this is dup from other files
...
# -------------------------------------------------
# Print iterations progress  (TODO: change to re-usable as others exist)
...
if __name__ == "__main__":
    ...
    for index_gdf_int, row_gdf_int in gdf_aoi_lambert.iterrows():
        ...
        str_URL_query_1 = r"SERVICE=WCS&VERSION=1.0.0&REQUEST=GetCoverage"
        r"&coverage=DEP3Elevation&CRS=EPSG:3857&FORMAT=GeoTiff"
        ...
        for index, row in gdf_tiles_intersect_only.iterrows():
            ...
            str_bbox = f"{str(b_tile[0])},{str(b_tile[1])},{str(b_tile[2])},{str(b_tile[3])}"
