...
# -------------------------------------------------
def fn_get_entwine_dem_from_shp(
    str_input_shapefile_path,
    str_output_path,
    b_in_feet,
    flt_search_distance,
    str_field_name,
    ...
    for i in range(len(gdf_boundary_lambert)):
        ...
        gdf_entwine_footprints_clip = gpd.overlay(
            gdf_entwine_footprints, gdf_boundary_lambert_single, how="intersection"
        )
        ...
        if len(gdf_entwine_footprints_clip) > 0:
            ...
            str_limits = (
                "Classification[2:2], Classification["
                + str(int_bridge_lidar_class)
                + ":"
                + str(int_bridge_lidar_class)
                + "]"
            )
            ...
            pipeline_dem_ground = {
                "pipeline": [
                    ...
                    {"type": "filters.crop", "polygon": boundary_geom_WKT.wkt},
                    {"type": "filters.range", "limits": str_limits, "tag": "Ground_Bridge"},
                    {
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="=============== GET ENTWINE DEM FROM SHAPEFILE ============="
    )

    parser.add_argument(
        "-i",
        dest="str_input_shapefile_path",
        help="REQUIRED: path to the rquested limits polygon shapefile Example:"
        r" C:\temp\terrain_limits_AR.shp",
        required=True,
        metavar="FILE",
        type=str,
    )
