...
class RAS2FIM_logger:
    ...
    def __init__(self):
        ...
        # logger.add(sys.stderr, format=":::{message}", filter=trace_filter)

        # console_trace_format = (
        ...
        # logger.add(sys.stderr, format=":::{message}", filter=trace_filter)
        # logger.add(sys.stdout, format=":::{message}", level="TRACE")

        # logger.add(sys.stderr, format=":::{message}",
        #           filter=lambda record: record["level"].name == "TRACE")

        # logger.add(logger.trace, format=":::{message}",
        #           filter=lambda record: record["level"].name == "TRACE")

        logger.add(sys.stderr, format=console_non_trace_format, level="DEBUG")
        # filter=lambda record: record["level"].name != "TRACE")

        # logger.trace.__format__ = ":::{message}"
        # logger.add(sys.stderr, format=":::{message}",
        #           filter="TRACE")
        # logger.add(sys.stdout, format=":::{message}", level="TRACE")
        logger.add(
    ...
    def setup(self, unit_output_folder: str, log_file_name: str = "ras2fim.log"):
        ...
        try:
            ...
            # Also add a custom log which just prints to console (like reg print)
            # and has no formatting, but also to log  (LPRINT (Log Print))
            # level_reg_print = logger.level("LPRINT", no=2)
            # logger.add(def_log_file, format = ":::{message}", filter=level_filter(level="LPRINT"))

            # By default, file and console logs only level "DEBUG" (10) and higher,
            # and for file logs, we want it all (inc TRACE).
            # (see __init__ were we setup the "debug" default level)

            # TRACE goes to screen and log but with no formatting
            # logger.add(def_log_file, format=":::{message}", level="TRACE")

            # DEBUG and higher goes to screen and log but with some formatting
            # logger.add(def_log_file, format=file_logger_format,
            #           filter=level_filter(level="TRACE"), enqueue=True, mode="w")
            # logger.add(def_log_file, format=file_logger_format,
            #           filter=log_filter, enqueue=True, mode="w")

            # logger.add(def_log_file, format=":::{message}", level="TRACE",
            #         filter=lambda record: record["level"].name == "TRACE")

            # logger.add(def_log_file, format=file_logger_format,
            #           level="DEBUG", enqueue=True, mode="w")
            ...
            # try catchign standard out
            # logger.add(sys.stdout, format = ":::{message}", level="LPRINT")
            # logger.add(sys.stdout, format = ":::{message}")
            # logger.add(def_log_file, format = ":::{message}", level="LPRINT")
    ...
    # -------------------------------------------------
    def add_custom_log(self, key_name, file_path_and_name):
        """
        Overview:
        ...
        Inputs:
          ...
          - file_path_and_name: the folder and file name to be used.
            ie)  C:\ras2fim_data\output_ras2fim\12030105_2276_231017\final\model_list.csv.
        ...
        folder_path = os.path.basename(file_path_and_name)
    ...
    def write_c_log(self, key_name, msg):
        """
        Overview:
          - add_custom_log must be called exactly once to setup the key and file path so
            the key here will work.
          - It is fine to have an empty msg logged if you like. You can log anything you like.
        """
