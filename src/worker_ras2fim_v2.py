# Create flow file for water surface elevation (WSE) Boundary Condition (BC)
#
...
# 12090301: 8 wse   #*12030202: Trinity, 8 wse  #12030105: Trinity, 3 wse #12040101: no wse bc:
huc8_num = "12090301"
# -------------------------------------------------
# Creating a new directry for each parent ras model with the new flow file


def create_directory_for_parent_ras_v2(path_to_ras2fimV2, huc8_num):
    # reads the name of all folders in parent ras model path
    # folder_names_v1 = os.listdir(path_to_parent_ras)

    # make a directory for parent model v2
    path_to_ras2fimV2_huc8 = path_to_ras2fimV2 + "/" + "ras2fim_v2_models_" + huc8_num
    os.mkdir(path_to_ras2fimV2_huc8)

    # #Copy and paste all parent ras models in the new directory
    # for folders in folder_names_v1:

    #     source = path_to_parent_ras + "/" + folders
    #     destination = path_to_ras2fimV2_huc8 + "/" + folders
    ...
    # #removing all old flow files in the new directory
    # old_flow_file_path_v2 = path_to_ras2fimV2_huc8 + "/*/*.f01"
    # ls_old_flow_file_path_v2 = glob.glob(old_flow_file_path_v2, recursive=True)

    # for old_flowf in ls_old_flow_file_path_v2:
    #     os.remove(old_flowf)


path_to_parent_ras = "C:/ras2fim_data/OWP_ras_models/models-12090301"
path_to_ras2fimV2 = "C:/ras2fim_data/OWP_ras_models/ras2fimv2.0"
create_directory_for_parent_ras_v2(path_to_ras2fimV2, huc8_num)


# -------------------------------------------------
# Reading old parent models flow and geometry files with WSE and normal depth (ND, slope) BCs
# and create a seperate list of flow and geometry files WSE and ND BCs
def create_list_of_flow_geometry_files_4each_BCs(path_to_parent_ras):
    path_flowfiles = path_to_parent_ras + "/*/*.f01"
    ls_path_flowfiles = glob.glob(path_flowfiles, recursive=True)
...
[
    ...
    str_path_to_geo_file_wse,
    str_path_to_geo_file_nd,
] = create_list_of_flow_geometry_files_4each_BCs(path_to_parent_ras)


# -------------------------------------------------
# Compute BC for the parent RAS models with WSE BC
def compute_boundray_condition_wse(str_path_to_flow_file_wse, str_path_to_geo_file_wse):
    ...
    for path_in in range(len(str_path_to_flow_file_wse)):
        ...
        # bc_target_xs_nm_col = pd.concat(
        #     [flows1st_target_xs, pd.DataFrame(pred_wse_flows1st_target_xs)], axis=1
        # )
        # bc_target_xs_nm = bc_target_xs_nm_col.set_axis(['discharge', 'wse'], axis=1)
...
# model = 8
# plt.plot(list_bc_target_xs_huc8[model]['discharge'],list_bc_target_xs_huc8[model]['wse'])
# plt.show()
...
def create_ras_flow_file_nd(
    ...
    for path_in in range(len(str_path_to_flow_file_nd)):
        ...
        path_newras_nd = (
            str_output_filepath + "/" + "ras2fim_v2_models_" + huc8_num + "/" + path_to_flow_file_nd_splt[1]
        )
...
str_output_filepath = "C:/ras2fim_data/OWP_ras_models/ras2fimv2.0"
...
def create_ras_flow_file_wse(
    huc8_num, str_path_to_flow_file_wse, str_output_filepath, profile_names, list_bc_target_xs_huc8
):
    int_fn_starting_flow = 1  # cfs
    int_number_of_steps = 76
    ...
    for path_in in range(len(str_path_to_flow_file_wse)):
        ...
        path_newras_wse = (
            str_output_filepath + "/" + "ras2fim_v2_models_" + huc8_num + "/" + path_to_flow_file_wse_splt[1]
        )
...
# TODO: Write a function for plotting
# TODO: Write the main function for this step
# TODO: Add ras2fime v1 error functions
# TODO: Add new rob error functions

# -------------------------------------------------
# TODO: plotting
# string_title = str_path_to_flow_file_wse[path_in]
# pattern = "\\"
# lst_ptr = string_title.rfind(pattern)
# tit_in = lst_ptr + 1
# graph_title = string_title[tit_in:-4]

# plt.plot(bc_target_xs['discharge'], bc_target_xs['wse'], color='green', lw = 4)
# plt.plot(bc_target_xs_nm['discharge'], bc_target_xs_nm['wse'], '>', color='purple')
# plt.plot(bc_observ_flow_wse_df['discharge'], bc_observ_flow_wse_df['wse'], 'o', color='black', ms = 10)
# plt.figtext(0.65, 0.71, f'R2 = {r2_2}', size = 11)
# plt.figtext(0.33, 0.55, f'R2 = {r2_1}', size = 11)
# plt.legend(["Monotonic RC", "Generated RC", "Observed"])
# plt.xlabel('Discharge (cfs)', size = 10)
# plt.ylabel('WSE (ft)', size = 10)
# plt.title(graph_title, size = 14)
# plt.savefig(f'C:/Users/rdp-user/Projects/dev-ras2fim2-create-flow-file-old/graphs_wse/{graph_title}_tri3.png')
# plt.show()


# -------------------------------------------------
# TODO: tofindconflicts = pd.DataFrame([name[:-11] for name in folder_names_v1])
# TODO: tofindconflicts.to_csv("C:/ras2fim_data/OWP_ras_models/ras2fimv2.0/12090301_conflict.csv")
# TODO: conflicts = tofindconflicts[tofindconflicts.duplicated()]
