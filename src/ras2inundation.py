...
def produce_inundation_from_geocurves(geocurves_dir, flow_file, output_inundation_poly, overwrite):
    ...
    Args:
        ...
        output_inundation_poly (str): Path to output inundation polygon.
    ...
    if not os.path.exists(os.path.split(output_inundation_poly)[0]):
        print("Parent directory for " + os.path.split(output_inundation_poly)[1] + " does not exist. Directory/ies will be created.")
    ...
    for feature_id in flow_file_df['feature_id']:
        if str(feature_id) not in available_feature_id_list:  # Skip flow values not found in RAS library
            continue
        ...
        if os.path.exists(polygon_path):
            feature_id_polygon_path_dict.update({feature_id: {'discharge_cms': discharge_cms, 'path': polygon_path}})
...
if __name__ == '__main__':
    ...
    parser.add_argument('-t', '--output_inundation_poly', help = 'Path to output inundation polygon file.', required = False)
