...
def vectorize(mosaic_features_raster_path, changelog_path, model_huc_catalog_path, rating_curve_path):
    ...
    # Because ras2fim loads from the 'models' directory and not the model_catalog, it is possible
    # to have rating curve values with no entry in models_catalog which means we are missing some
    # meta data. The key is a left join with the rating curve as the base.
    ...
    # don't confuse our Hydrovis end users.
    ...
    cols_to_drop = []
    if "nhdplus_comid" in gdf.columns:
        cols_to_drop.append("nhdplus_comid")
    if "g_file" in gdf.columns:
        cols_to_drop.append("g_file")
    if "crs" in gdf.columns:
        cols_to_drop.append("crs")
    if "nhdplus_inital_scrape_namecomid" in gdf.columns:
        cols_to_drop.append("inital_scrape_name")
    if "nhdplus_initial_scrape_namecomid" in gdf.columns:
        cols_to_drop.append("initial_scrape_name")
    ...
    # where would we find this in the model data?
    gdf["model_date"] = ["Unknown"] * len(gdf)
...
def __get_maxment(mxmt_args):
    ...
    max_rem = -1
    max_rem_file = ""
    # Look through all depth grid tiffs associated with this feature ID and find the
    # max depth value and file.
    for file_name in feature_id_tif_paths:
        # cut off the .tif at the end.
        adj_file_name = str(file_name).replace(".tif", "")

        # get rem (depth) value from filename (last segment)
        depth_value = int(adj_file_name.split("-")[-1])
    ...
    masked_raster, transform = rasterio.mask.mask(
        feature_max_depth_raster,
...
def __validate_make_catchments(huc_num, r2f_huc_parent_dir, model_huc_catalog_path, national_ds_path):
    ...
    if huc_num.isnumeric() is False:  # can handle leading zeros
    ...
    if is_invalid_path is True:
        raise FileNotFoundError(
            "The -o arg (parent [ras2fim HUC output] ) folder does not exist. Please correct and retry."
        )
    rtn_varibles_dict["r2f_huc_parent_dir"] = r2f_huc_parent_dir

    # -------------------
    # -n  (ie: inputs\\X-National_Datasets)
    if os.path.exists(national_ds_path) is False:
        raise FileNotFoundError(
            "the -n arg (inputs x national datasets path arg) does not exits. Please correct and retry."
        )
    ...
    if os.path.exists(r2f_06_metric_dir) is False:
        raise FileNotFoundError(
            f"The ras2fim huc output, {sv.R2F_OUTPUT_DIR_METRIC} subfolder does not exist."
    ...
    if os.path.exists(r2f_catchments_dir) is True:
...
def make_catchments(
    ...
    - r2f_huc_parent_dir : str
        The partial or full path to the ras2fim output HUC directory. That folder must already have
        a fully populated with the "05_" depth grid tifs.
        This value can be the value of just the the output_ras2fim huc subfolder,
        ie 12090301_meters_2277_test_3.
           (We will use the root default pathing and it becomes
            c:/ras2fim_data/outputs_ras2fim_models/12090301_meters_2277_test_3)

        OR it can be a full path to the ras2fim huc output folder.
        ie) c:/my_ras2fim_outputs/12090301_meters_2277_test_3.
        Either way, it needs at least the populated 05_hecras_output subfolder.

    - national_ds_path : str
        This needs to be the full path of the X-National_Dataset fo

    - model_huc_catalog_path : str
        This is the location of a models_catalog.csv or a subset of it. When the new get models by huc
        comes online shortly, it will create a file called "OWP_ras_models_catalog_{HUC}.csv.
        By default, that will be used here with the pattern in code being "models_catalog_[].csv" and
        will use subsitution to replace the [] with the huc number. This is an optional input override
        but will default to c:/ras2fim_data/OWP_ras_models same as other systems.

    """

    rtn_varibles_dict = __validate_make_catchments(
    ...
    if os.path.exists(r2f_rem_path):
        ...
        with rasterio.open(r2f_rem_path) as src:
            ...
            output_meta.update({"dtype": np.int32, "compress": "LZW", "crs": proj_crs})
    ...
    if (is_verbose is False) and (os.path.exists(datatyped_rems_dir)):
        shutil.rmtree(datatyped_rems_dir)
        # shutil.rmtree is not instant, it sends a command to windows, so do a quick time out here
        # so sometimes mkdir can fail if rmtree isn't done
...
if __name__ == "__main__":
    ...
    #     python ras2catchments.py -w 12090301
    #          -o C:\ras2fim_data_rob_folder\output_ras2fim_models_2222\12090301_meters_2277_test_2
    #          -n E:\X-NWS\X-National_Datasets -mc c:\mydata\robs_model_catalog.csv

    #  - The -p arg is required, but can be either a full path (as shown above), or
    #        a simple folder name.Either way, it must have the and the 05_hecras_output and
    #        the 06_ras2rem folder and populated.
    #        ie) -o c:/users/my_user/desktop/ras2fim_outputs/12090301_meters_2277_test_2
    #            OR
    #            -o 12090301_meters_2277_test_2  (We will use the root default pathing and
    #            it becomes c:/ras2fim_data/outputs_ras2fim_models/12090301_meters_2277_test_2)

    parser = argparse.ArgumentParser(
        description="========== Create catchments for specified existing output_ras2fim folder =========="
    )
    ...
    parser.add_argument(
        ...
        r" such as c:\users\my_user\Desktop\myoutput OR you can add a simple ras2fim output"
        " huc folder name. Please see the embedded notes in the __main__ section of the code for"
    ...
    parser.add_argument(
        ...
        help=r"OPTIONAL: path to model catalog csv, filtered for the supplied HUC, file"
        r" downloaded from S3. Defaults to c:\ras2fim_data\OWP_ras_models\OWP_ras_models_catalog_[].csv"
    ...
    parser.add_argument(
        "-v",
        dest="is_verbose",
        help="OPTIONAL: if this flag is add (no value required), additional output files will be saved"
        "  and extra terminal window text will be displayed (for debugging purposes)",
        default=False,
        required=False,
        action="store_true",
    )
